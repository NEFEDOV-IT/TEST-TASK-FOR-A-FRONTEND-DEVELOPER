      ...
      <div className="people__list">
        {isLoading ? <Loader/> :
          <List items={users} renderItem={(user: IUser) =>
            <PeopleItem onClick={(user) => navigate('person/' + user.id)} user={user} key={user.id}/>}
          />
        }
        {isLoading || <div className="people__total">{`Найдено ${(users as IUser[]).length} пользователей`}</div>}
      </div>
    </div>
  );
};
